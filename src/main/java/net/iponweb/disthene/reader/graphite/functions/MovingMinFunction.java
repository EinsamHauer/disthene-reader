 ...
 */
public class MovingMinFunction extends DistheneFunction {
        ...
        for (int i = 0; i < processedArguments.size(); i++) {
            ...
            for (int j = 0; j < bootstrappedLength; j++) {
                if (queue.size() == 0) {
                    ...
                    values[j] = CollectionUtils.min(queue);
                }
            ...
            processedArguments.get(i).setName("movingMin(" + processedArguments.get(i).getName() + "," + window + ")");
        }
