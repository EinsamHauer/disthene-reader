 ...
 */
public class MovingMaxFunction extends DistheneFunction {
        ...
        for (int i = 0; i < processedArguments.size(); i++) {
            ...
            for (int j = 0; j < bootstrappedLength; j++) {
                if (queue.size() == 0) {
                    ...
                    values[j] = CollectionUtils.max(queue);
                }
            ...
            processedArguments.get(i).setName("movingMax(" + processedArguments.get(i).getName() + "," + window + ")");
        }
