 ...
 */
public class MaxSeriesFunction extends DistheneFunction {


    public MaxSeriesFunction(String text) {
        super(text, "maxSeries");
    }
    ...
    public void checkArguments() throws InvalidArgumentException {
        if (arguments.size() < 1) throw new InvalidArgumentException("maxSeries: number of arguments is " + arguments.size() + ". Must be at least one.");

        for(Object argument : arguments) {
            if (!(argument instanceof Target)) throw new InvalidArgumentException("maxSeries: argument is " + argument.getClass().getName() + ". Must be series");
        }
    }
}
